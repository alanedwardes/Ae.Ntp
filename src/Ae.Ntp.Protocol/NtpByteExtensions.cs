...
{
    internal static class NtpByteExtensions
    {
        ...
        public static void ToBytes(ReadOnlyMemory<byte> value, Memory<byte> buffer, ref int offset)
        {
            value.CopyTo(buffer.Slice(offset));
            offset += value.Length;
        }

        public static byte ReadByte(ReadOnlyMemory<byte> bytes, ref int offset)
        {
            var b = bytes.Span[offset];
            offset += 1;
            return b;
        }

        public static NtpShort ReadNtpShort(ReadOnlyMemory<byte> bytes, ref int offset)
        {
            return new NtpShort
            {
                Seconds = ReadUInt16(bytes, ref offset),
                Fraction = ReadUInt16(bytes, ref offset)
            };
        }

        public static void ToBytes(NtpShort value, Memory<byte> buffer, ref int offset)
        {
            ToBytes(value.Seconds, buffer, ref offset);
            ToBytes(value.Fraction, buffer, ref offset);
        }

        public static NtpTimestamp ReadNtpTimestamp(ReadOnlyMemory<byte> bytes, ref int offset)
        {
            return new NtpTimestamp
            {
                Seconds = ReadUInt32(bytes, ref offset),
                Fraction = ReadUInt32(bytes, ref offset)
            };
        }

        public static void ToBytes(NtpTimestamp value, Memory<byte> buffer, ref int offset)
        {
            ToBytes(value.Seconds, buffer, ref offset);
            ToBytes(value.Fraction, buffer, ref offset);
        }
        ...
        public static ReadOnlyMemory<byte> AllocateAndWrite(INtpByteArrayWriter writer)
        {
        ...
        public static TReader FromBytes<TReader>(ReadOnlyMemory<byte> bytes) where TReader : INtpByteArrayReader, new()
        {
            var offset = 0;
        ...
        public static void ToBytes(sbyte value, Memory<byte> buffer, ref int offset)
        {
            buffer.Span[offset++] = (byte)value;
        }

        public static void ToBytes(byte value, Memory<byte> buffer, ref int offset)
        {
            buffer.Span[offset++] = value;
        }

        public static byte[] ToBytes(this INtpByteArrayWriter writer)
        {
            var buffer = AllocatePinnedNetworkBuffer();
